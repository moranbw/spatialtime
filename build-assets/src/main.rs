...
static OSM_SOURCE_ADDRESS: &str = "https://github.com/evansiroky/timezone-boundary-builder/releases/latest/download/timezones-with-oceans-now.geojson.zip";
static NED_SOURCE_ADDRESS: &str = "https://raw.githubusercontent.com/nvkelso/natural-earth-vector/master/geojson/ne_10m_time_zones.geojson";
...
fn main() -> Result<()> {
    ...
    let ned_path = parent_path.join("assets").join("timezones_ned.fgb.zst");
    let ned_bytes = get_ned_bytes()?;
    write_fgb(ned_bytes, ned_path)?;

    let osm_path = parent_path.join("assets").join("timezones_osm.fgb.zst");
    let osm_bytes = get_osm_bytes()?;
    write_fgb(osm_bytes, osm_path)?;

    Ok(())
}

fn get_ned_bytes() -> Result<Vec<u8>> {
    let mut response = reqwest::blocking::get(NED_SOURCE_ADDRESS)?;
...
fn get_osm_bytes() -> Result<Vec<u8>> {
    let response = reqwest::blocking::get(OSM_SOURCE_ADDRESS)?;
