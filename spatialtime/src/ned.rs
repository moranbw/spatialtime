...
pub fn lookup(longitude: f64, latitude: f64) -> Result<SpatialtimeResponse, SpatialtimeError> {
    ...
    Ok(SpatialtimeResponse { offset: Some(offset), tzid })
}
...
pub fn ned_test() {
    ...
    assert_eq!(lookup(white_house.x(), white_house.y()).unwrap().offset.unwrap(), -5.0);
    assert_eq!(lookup(the_lodge.x(), the_lodge.y()).unwrap().offset.unwrap(), 10.0)
}